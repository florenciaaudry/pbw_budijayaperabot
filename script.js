...
chips.forEach(function (chip) {
  chip.addEventListener("click", function () {
    ...
    // reset semua chip dulu
    chips.forEach(function (c) { c.classList.remove("active"); });

    if (cat === "all") {
      // pilih "Semua" → kosongkan filter kategori
      state.categories = [];
    ...
    // single-select: pilih satu kategori saja
...
function syncChipUIFromState() {
  chips.forEach(function (chip) {
    var cat = chip.getAttribute("data-cat");
    if (cat === "all") {
      chip.classList.toggle("active", !state.categories || state.categories.length === 0);
    } else {
      chip.classList.toggle("active", state.categories.length === 1 && state.categories[0] === cat);
    }
  });
}
...
// ==========================
// Checkout ditangani oleh checkout_patch.js
// Expose cart & helpers ke window agar checkout_patch.js bisa akses
...
window.getCartItems = function() {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (prod) result.push({ id: prod.id, name: prod.name, price: prod.price, qty: qty });
  }
  return result;
};
...
// Auto-buka keranjang jika diarahkan dari halaman profil
if (sessionStorage.getItem("pbw_open_cart") === "1") {
  sessionStorage.removeItem("pbw_open_cart");
  openCart();
}
